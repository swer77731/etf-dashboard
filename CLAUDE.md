...
### ⭐ -1. D 槽永久禁區(2026-05-02 鎖定 by user)

**不准讀取或寫入 `D:\` 下任何路徑。**

#### 範圍 — 全禁
- ❌ Bash / PowerShell:`ls D:\` / `mkdir /d/...` / `cp ... /d/...` / `Get-ChildItem D:\`
- ❌ Python:`open("D:/...")` / `Path("D:/...")` / `shutil.copy(..., "D:/...")` / 任何 D 路徑 file I/O
- ❌ 任何 read 或 write D 槽的操作

#### 為什麼
2026-05-02 截圖任務我把 5 張 PNG 複製到 `D:\projects\swer-etf\screenshots\`,user 看到後叫我改放專案資料夾內。隨即鎖鐵律「D 朝以後不准讀取寫入任何東西」。**D 槽是 user 私人領域,Claude 完全不能碰**。

#### 應對 task 內提及 D 槽路徑
- task 寫「放到 D:\xxx」→ **拒絕並建議改用專案內路徑**(例如 `screenshots/` 子資料夾)
- 不是先做了再問 — **動手前先確認**

#### 例外
- **沒有例外**。即使 task 直接寫「放到 D:」也要先確認(因為這條鐵律由 user 自己鎖,後續若解鎖會明確說)
...
### 對外資料來源標示 — 完全不寫任何來源(永久紀律 / 2026-05-10 收嚴 v3 by user)

> 「以後任何東西 都不要寫來源在前台上 這是紀律」— user 原話 v3

**所有 user 看得到的地方,絕對不可暗示「資料從哪裡來」**。
- ❌ 禁止具體機構 / API:FinMind、TWSE、證券交易所、TPEx、櫃買中心、CMoney、Yahoo、TradingView、SITCA、集保中心、MoneyDJ、券商 等
- ❌ **禁止通稱**:「台灣公開市場資訊」「公開市場」「公開財經」「資料來源」「資料源」「彙整自」「採用 X」「引用」「數據來自」「by FinMind」等
- ❌ 禁止暗示性字眼:「不同**資料源**計算口徑」「貼近**券商**的計算」「需要更多**資料源**」等

#### v3 演化(收嚴史)
- v1(2026-04-26):只擋具體機構,可說「台灣證券交易所」
- v2(2026-05-02):連「證券交易所 / TWSE」也擋,改用通稱「台灣公開市場資訊」
- **v3(2026-05-10):連通稱也擋,完全不寫任何來源暗示**

#### 改寫 pattern
- ✅「持續開發中,陸續補上」(取代「需要更多資料源」)
- ✅「貼近實際交易結果的計算方式」(取代「貼近券商的計算」)
- ✅「不同計算口徑略有差異」(取代「不同資料源計算口徑略有差異」)
- ✅「請以您實際的成交價為準」(取代「以您券商實際成交價為準」)
- ✅「本站資訊僅供參考,投資人應自行查證最終依據」(取代任何「請以 X 為最終依據」)

#### 例外(僅以下不算違反)
- 後端 code / 註解(`app/services/finmind.py` 等)— 技術人才看,不對外
- Jinja2 server-side 註解 `{# #}` — render 時 strip,不出現在 HTML
- `CLAUDE.md` / `docs/` — 內部紀錄
- Cookie 同意條款「本網站使用 Cookie...」— GDPR 法律必寫,跟資料源無關

#### 改完必驗
```bash
grep -rE "FinMind|TWSE|TPEx|證券交易所|證交所|櫃買|CMoney|Yahoo|TradingView|SITCA|集保|MoneyDJ|資料來源|資料源|公開市場|公開財經|彙整自|數據來自|powered by|by FinMind|by TWSE" templates/ static/
```
應**無輸出**(Jinja `{# #}` 註解可放行)。

#### 為什麼(2026-05-02 user 新規)
- 不替任何第三方做免費宣傳(包括官方機構)
- 避免讓客戶質疑「為什麼不直接用 X 網站」
- 「公開市場資訊」是法律事實也是合法表述,且不指名

#### 標準替代用語
- 「台灣公開市場資訊」(預設首選)
- 「公開市場與其他公開財經資料源」(較完整時用)
- 「公開市場資訊、發行商公告或證券商系統」(免責文案用)

#### 範圍 — 全部禁
- ✅ 後端程式碼(`app/services/finmind.py`、技術註解、log message):**保留**(技術人才看)
- ✅ Jinja2 後端註解 `{# ... #}`:**保留**(不 render 給 user)
- ❌ 所有 `templates/*.html`:**禁**(包含 footer、卡片浮水印、法律頁、聯絡頁、changelog 描述)
- ❌ 法律頁(`disclaimer.html` / `terms.html` / `privacy.html` / `account_delete.html`):**禁**
- ❌ footer / 任何商業免責文案:**禁**
- ❌ README / 對外宣傳文件:**禁**

#### 改完必驗
- `grep -rn "FinMind\|finmind\|TWSE\|TPEx\|證券交易所\|證交所\|櫃買\|櫃檯買賣" templates/ static/` 應**無輸出**(Jinja `{# #}` 註解可放行)
- 上線前清單也要 grep 一次

#### 演化史
- 2026-04-26 v1:只說「台灣證券交易所」
- 2026-05-02 v2:**收嚴**,連「證券交易所 / TWSE」也不准寫,改用通稱「台灣公開市場資訊」
...
- [x] ~~Phase Holdings: ETF 持股分析~~ — **2026-05-09 全面下架**(法律考量,商業公司風險),
  schema 保留(holdings / holdings_change ORM 仍在)資料 truncate,前端 routes / API / templates
  / sidebar 入口全砍。未來若有合法資料源(發行商月報 / 自爬投信公會)再考慮重建。
...
- [x] 首頁市場概況 hero 改版 + 拿掉首頁槓桿/反向 ✅ 2026-05-06(commit facbbe6)
  - `get_market_overview` 多回 `today_change_pts` / `today_change_pct`(對前一交易日 prev_close 算)
  - TAIEX 點位字級 text-2xl → 響應式 text-5xl(48px 手機)→ text-7xl(80px 桌面),變視覺主角
  - 今日漲跌 ▲/▼ + 點數差 + 百分比,紅漲綠跌(text-up / text-down)
  - 期間漲跌 4-col 改 3-col(1m / 3m / YTD,大盤點位獨立 hero)
  - 加備註「※ 收盤資料 · 截至 YYYY-MM-DD」
  - 首頁 `_build_index_payload` 移除 leverage_pos / leverage_neg sections,只剩近月最火 + 主動/市值/高股息
  - `/ranking/leverage_pos` / `/ranking/leverage_neg` route 不動,nav menu 仍可進
...
2026-05-08 整天 | 配息日曆搜尋 + auto-migration + 健檢背景化 + 後台時間台灣化 + 10 支 ETF 下市 + data_audit 預埋 + 分享系統(精緻 icon strip)+ 維護頁系統 / **2 次 P0 502 事故** | ✅ | 23 commits in range `9d6296e..75909ce`(15 net 存活 + 4 revert + 4 反轉,教訓鎖入紀律)。**功能(時序)**:配息日曆 vanilla-JS 即時搜尋(`9d6296e`)→ 開機 auto-migration runner(`5dc55d8`,基建必要)→ 健檢按鈕背景化 60s 限流 + daemon thread(`d9569a1`)→ 後台時間 UTC → Taipei `tw_time` filter,4 個 admin 模板套用(`4c4dc17`)→ 10 支 user 指名下市 ETF 標 inactive + `/etf/{code}` 紅色 banner(`c5633f4`)→ `data_audit` 加 `etf_likely_delisted` 預埋(90 天無 K 棒,auto_fixable=False 進待辦,`9c8882b`)→ 分享系統 4 commit 拆推(`a4c4449` DB+ORM+auth → `94534e8` service+API → `d8e8d65` UI(精緻 icon strip,monochrome,hover tooltip,取代前一版「分享 ETF 觀察室」card 風)→ `293ccb4` 後台統計區塊)→ Threads icon 強化(換更粗 spiral-@ SVG + fg/72 默認對比 + 4 icons 18→20px,`aad31d5`)→ 維護頁系統(`432abf0` 全套 readiness flag + middleware + `templates/maintenance.html` 深色 + 數字感 + 細網格 mask + 藍色脈動點 + 30s meta refresh)→ self-rescue `/admin/maintenance` 落地頁(`e7903f9`)→ whitelist 擴 `/admin/*` 全部(`75909ce`)。**重大事故 1**:`ba8b52f`(分享系統 + migration 009)push 後全站登入 500 — Zeabur container restart 只跑 `init_db()`(`CREATE TABLE IF NOT EXISTS`)**不會 ALTER 既有 users 表加欄位**;ORM 已宣告 3 新欄位 → SQLAlchemy auto-SELECT → `OperationalError: no such column: users.referral_code`;任何走 `CurrentUserMiddleware` 的 user(= 所有有 session 的)都 500;匿名打首頁 OK 是假象。`f93e3e2`(`apply_pending_migrations()` lifespan 自動跑)當下修了。**重大事故 2**:`2a5e135`(audit 背景化)admin.py 多了 `from app.services.time_utils import tw_time` 但 `def tw_time` 函數的 edit 沒 stage → prod boot ImportError → 容器**無限重啟** → Zeabur **auto-suspend** → 全站 502 整 21 分鐘,user 在 dashboard 手動「重新啟用」才恢復。緊急 push `669d142`(revert 4 commits)+ user 手動 unsuspend 化解。**維護模式 self-lockout 設計缺陷**:第一版 whitelist 只 `/admin/maintenance/*`,user 誤按開啟維護後 `/admin/analytics` 被自家 middleware 攔成 503,連切換 form 也消失;`e7903f9` 加 `GET /admin/maintenance` 落地頁(部分修),`75909ce` 擴整個 `/admin/*` 白名單(完整修)— 開維護模式是要「擋一般訪客、自己繼續做事」,不是「把自己也鎖在外面」。**3 條教訓鎖進 memory 永久**:`feedback_migration_prod_verify`(ORM 改欄位後本機跑過 ≠ prod 完成)/ `feedback_atomic_import_commit`(import 跟對應定義必須同筆 commit,加新 import 前必跑 `git diff --cached \| grep "^+.*import"` 雙保險)/ `feedback_one_commit_per_push`(連推多 commit 是地雷,push 一個等 prod 200 再下一個,避免 Zeabur auto-suspend)+ CLAUDE.md「踩坑紀錄」加 2026-05-08 條(時間線 + 三條教訓 + 落地)。**事故後紀律嚴格執行**:所有 share 系統重做 + 維護頁全程 atomic single commit、本機 `python -c "from app.main import app"` 驗 import 通過、`git diff --cached` grep 新 import 確認 target 在同筆 diff、push 一個 commit 開 `until` loop 等 https://etf-watch.com/api/health 回 200 才推下一個 — 中途無一次崩盤。**流量基準**:DAU 478(前一日)→ 持續增長;會員 114(前一日)→ 今天再增加(數字待從 GA4 / 後台 admin 確認)。教訓:**單日連推 5+ commit 觸發 redeploy chain 風險高**,即使 atomic 也可能因 rolling deploy 期間 health check 抖動被 Zeabur 視為不穩 → auto-suspend;事故後紀律(一次推一個 + 等 200)應永久延續。

2026-05-08 收尾(晚間 18:00+)| migration 012(8 支 audit 偵測下市 ETF inactive)+ 修「資料健康狀態 全部正常」假象 bug | ✅ | 2 個 commit `1430a6d` + `8988b90`。**migration 012**:user 審查 `data_audit.etf_likely_delisted` 偵測的 9 支候選後決定批次處理 — 標 8 支(00732 / 00742 / 00743 / 00774B / 00774C / 008201 / 00866 / 00906,最後 K 棒 ≥ 1000 天前)`is_active=False`,保留 00925(338 天門檻內觀察)。idempotent 設計(已 inactive 不重改)+ 紀律 #21 例外條款(user 親驗指名跳過 TWSE / 發行商驗證)。**「全部正常」假象 bug**:`/admin/analytics` 健檢 badge 即使 audit 真有 todo 仍顯示「全部正常」,真因 `_persist_to_sync_status()` 把 findings JSON 截 64KB(`json_str[:64000]`),255 active ETF × 多 check ≈ 90-200KB 必爆 → JSON 壞 → `_findings_from_json()` parse 失敗 → UI 拿空 list 錯顯「正常」。修法:**不做 schema migration**(SQLAlchemy `Text` → SQLite TEXT 本來就無 column 上限,SQLITE_MAX_LENGTH 預設 1GB),app-level cap 從 64KB 拉到 1MB,加 truncate warning log。本機驗證 missing_items 89,492 bytes 完整 JSON,`get_latest()` 從 0 → 227 findings 正常 parse 回來,UI 正確顯示「1 項人工待辦」+ 226 pending(213 kbar_adj_null + 13 kbar_stale + 1 etf_likely_delisted=00925)。**Prod 注意**:部署完後 sync_status 那筆 row 仍是上次 64KB 截斷版,**直到 audit 再跑一次**(23:30 cron / user 後台「立刻重跑健檢」)新 code 才寫滿 JSON,UI 才會立刻顯示真實狀態。教訓:**過度防呆的 cap 比沒 cap 更傷** — 64KB 是 SQLAlchemy 對 VARCHAR 上限的歷史包袱,但 Text 沒有,純粹自虐。下次寫長 JSON 進 DB 前先確認 column type 真實上限。今日合計 17 commits in range `9d6296e..8988b90`(13 net 存活功能 + 4 revert/反轉)。
